...
import { Beef, Heart, Loader2, ShoppingBag, Stethoscope, Trash2, TrendingDown, TrendingUp } from "lucide-react";
...
export default function Livestock() {
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      toast.success(`Achat: ${count} × ${catalog[buyOpen].name}`);
      setBuyOpen(null);
      setCount(1);
      await refresh();
      ...
      <section>
        ...
        {herds.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {herds.map((h) => {
              const cat = catalog[h.type];
              const prod = products[cat.produces];
              return (
                <div key={h.id} data-testid={`herd-${h.id}`} className="solid-card overflow-hidden">
                  <div className="relative h-32 bg-stone-200">
                    <img src={ANIMAL_IMG[h.type]} alt={cat.name} loading="lazy" className="w-full h-full object-cover" />
                  ...
                  <div className="p-5 space-y-3">
                    <div className="flex items-center justify-between text-xs">
                      <span className="flex items-center gap-1.5 text-stone-600">
                        ...
                        Santé: <strong className="text-stone-900">{h.health}%</strong>
                      </span>
                      <span className="text-stone-600">
                        Hier: <strong className="text-emerald-700">{(h.last_production ?? 0).toFixed(2)} {prod.unit}</strong> {prod.name.toLowerCase()}
                      </span>
                    </div>
                    <div className="text-xs text-stone-500">
                      Alimentation: {(cat.feed_cost_per_day * h.count).toFixed(1)} €/jour
                    </div>
                    <div className="flex gap-2 pt-2">
                      <Button size="sm" variant="outline" onClick={() => handleVet(h.id)} disabled={busy === `vet-${h.id}`} data-testid={`vet-${h.id}`}>
                        ...
                        Vétérinaire (~{18 * h.count}€)
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => handleSell(h.id, h.count)} disabled={busy === `sell-${h.id}`} data-testid={`sell-herd-${h.id}`}>
              ...
              <div key={key} data-testid={`buy-animal-${key}`} className="solid-card overflow-hidden hover:-translate-y-[2px] transition-all">
                ...
                <div className="p-5">
                  ...
                  <div className="text-xs text-stone-500 mt-1">
                    Produit: {prod.name} ({cat.daily_yield} {prod.unit}/jour)
                  </div>
        ...
        <DialogContent data-testid="buy-livestock-dialog">
          ...
          {buyOpen && (
            <div className="space-y-3">
              <div className="text-sm text-stone-600">{catalog[buyOpen].name} · {fmtCur(catalog[buyOpen].buy_price)}/unité</div>
              ...
              <div className="text-sm text-stone-700">Total: <strong>{fmtCur(catalog[buyOpen].buy_price * count)}</strong></div>
            </div>
          )}
          <DialogFooter>
